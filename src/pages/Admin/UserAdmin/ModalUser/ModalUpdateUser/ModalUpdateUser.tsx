...
export default function ModalUpdateUser({ closeModal, userId }: Props) {
  ...
  const userInfo = userInformation?.data.data;
  console.log('userInfo: ', userInfo);

  const updateUserMutation = useMutation({
    mutationFn: (body: UpdateUser) => userApi.updateUser(userId, body)
  });
  ...
  const onSubmit = handleSubmit(
    (data) => {
      const updateUserData = {
        ...
        age: Number(data.age),
        company:
          companyName.name || data.company
            ? {
                name: (companyName.name ? companyName.name : data.company) as string
              }
            : null,
        roles: [
          {
      ...
      console.log('check data: ', updateUserData);
    },
    (errors) => {
      console.log('check error submit: ', errors);
    }
  );
  ...
  useEffect(() => {
    if (userInfo) {
      ...
      setValue('roles', userInfo.roles[0].name);
