...
import { searchInvitationSchema, searchSchemaInvitation } from '../../../utils/rules.ts';
import { useForm } from 'react-hook-form';
import { yupResolver } from '@hookform/resolvers/yup';
import useQueryParams from '../../../hooks/useQueryPrams.tsx';
import useQueryConfig from '../../../hooks/useQueryConfig.tsx';
import { keepPreviousData, useQuery } from '@tanstack/react-query';
import invitationApi from '../../../apis/invitation.api.ts';

type FormData = Pick<searchSchemaInvitation, 'email'>;
const searchInvitationAdminSchema = searchInvitationSchema.pick(['email']);

export default function InvitationAdmin() {
  const [isOpenModalCreateInvitation, setOpenModalCreateInvitation] = useState<boolean>(false);
  const [isSearch, setSearch] = useState<boolean>(false);
  const navigate = useNavigate();
  const { register, handleSubmit, setValue } = useForm<FormData>({
    resolver: yupResolver(searchInvitationAdminSchema)
  });
  const queryParams = useQueryParams();
  const queryConfig = useQueryConfig();
  const queryConfigInvitationAdmin = {
    ...queryConfig,
    sortBy: 'createdAt',
    sortDir: 'desc',
    pageSize: '10'
  };
  const pageNo = Number(queryConfigInvitationAdmin.pageNo);
  const pageSize = Number(queryConfigInvitationAdmin.pageSize);

  const { data: invitationListData, isLoading: invitationListLoading } = useQuery({
    queryKey: ['invitationList', queryConfigInvitationAdmin],
    queryFn: () => invitationApi.getAllInvitations(queryConfigInvitationAdmin),
    placeholderData: keepPreviousData
  });
  const totalPagesInvitationList = Number(invitationListData?.data.data.meta.totalPages);

  const { data: invitationListSearchData, isLoading: invitationListSearchLoading } = useQuery({
    queryKey: ['invitationListSearch', queryParams],
    queryFn: () => invitationApi.searchInvitation(queryConfigInvitationAdmin),
    placeholderData: keepPreviousData,
    enabled: isSearch
  });
  const totalPagesInvitationListSearch = Number(invitationListSearchData?.data.data.meta.totalPages);
  const onSubmit = handleSubmit((data) => {
    setSearch(true);
    navigate({
      pathname: '/admin/invitation',
      search: createSearchParams({
        ...queryConfigInvitationAdmin,
  ...
  return (
    <div className='my-[54px]'>
      <div className='text-[18px] leading-7 font-medium text-[#18191c]'>Invitations</div>
      ...
      <div className='mt-[24px]'>
        <div className='grid grid-cols-12 bg-[#f1f2f4] px-[20px] py-[10px] rounded-[4px]'>
          ...
          <div className='col-span-3 text-[12px] text-[#535860]'>Sender User</div>
          <div className='col-span-3 text-[12px] text-[#535860]'>Receiver User</div>
      ...
      <div className='mt-2 h-[530px]'>
        {invitationListLoading ? (
          ...
          invitationListSearchLoading ? (
            <div className='h-[530px] flex items-center justify-center  '>
              <Loading />
            </div>
          ) : invitationListSearchData?.data.data.data && invitationListSearchData?.data.data.data.length > 0 ? (
            invitationListSearchData?.data.data.data.map((invitation, index) => (
              <div
                key={invitation.id}
        ...
        hover:cursor-pointer hover:outline outline-[#0b65cc] rounded-[8px]'
              >
                ...
                <div className='col-span-3 text-[16px] text-[#535860]'>{invitation.senderUser.email}</div>
                <div className='col-span-3 text-[16px] text-[#535860]'>{invitation.receiverUser.email}</div>
                <div className='col-span-2 text-[16px] text-[#535860]'>{invitation.company.name}</div>
                <div className='col-span-2 text-[16px] text-[#535860]'>{invitation.status}</div>
                <div
                  className='col-span-1 text-center text-[16px] text-[#535860] flex justify-center
          items-center gap-[6px]'
                >
                  <button
                    // onClick={(event) => handleClickButtonEdit(event, category.id)}
        ...
        ) : invitationListData?.data.data.data && invitationListData?.data.data.data.length > 0 ? (
          invitationListData?.data.data.data.map((invitation, index) => (
            <div
              key={invitation.id}
              className='grid grid-cols-12 px-[20px] py-[10px] items-center border-b
        hover:cursor-pointer hover:outline outline-[#0b65cc] rounded-[8px]'
      ...
      {invitationListLoading || invitationListSearchLoading ? (
        ...
        <div className='mt-[30px] flex items-center justify-end gap-x-[20px]'>
          {isSearch ? (
            <>
              ...
              {pageNo === totalPagesInvitationListSearch ? (
                <span className=''>Next</span>
              ) : (
                <Link
                  className='text-[#0b65cc]'
              ...
              <div className='w-[100px]'>
                Page {pageNo} / {totalPagesInvitationListSearch}
              </div>
            </>
          ) : (
            <>
              ...
              {pageNo === totalPagesInvitationList ? (
                <span className=''>Next</span>
              ) : (
                <Link
                  className='text-[#0b65cc]'
              ...
              <div className='w-[100px]'>
                Page {pageNo} / {totalPagesInvitationList}
              </div>
            </>
          )}
        </div>
