...
import { searchCategorySchema, searchSchemaCategory } from '../../../utils/rules.ts';
import { yupResolver } from '@hookform/resolvers/yup';
import ModalCreateCategory from './ModalCategory/ModalCreateCategory';
import ModalUpdateCategory from './ModalCategory/ModalUpdateCategory';

type FormData = Pick<searchSchemaCategory, 'name'>;
const searchCategoryAdminSchema = searchCategorySchema.pick(['name']);

export default function CategoryAdmin() {
  const [isOpenModalCreateCategory, setOpenModalCreateCategory] = useState<boolean>(false);
  const [isOpenModalUpdateCategory, setOpenModalUpdateCategory] = useState<boolean>(false);
  const [categoryId, setCategoryId] = useState<string>('');
  const [isSearch, setSearch] = useState<boolean>(false);
  const { register, handleSubmit, setValue } = useForm<FormData>({
    resolver: yupResolver(searchCategoryAdminSchema)
  });
  ...
  const pageNo = Number(queryCategoryAdminConfig.pageNo);
  const pageSize = Number(queryCategoryAdminConfig.pageSize);

  const { data: categoryListData, isLoading: categoryListLoading } = useQuery({
    queryKey: ['categoryList', queryCategoryAdminConfig],
    queryFn: () => categoryApi.getAllCategory(queryCategoryAdminConfig),
    placeholderData: keepPreviousData
  });
  const totalPagesCategoryList = Number(categoryListData?.data.data.meta.totalPages);

  const { data: categoryListSearchData, isLoading: categoryListSearchLoading } = useQuery({
    queryKey: ['categoryListSearch', queryParams],
    queryFn: () => categoryApi.searchCategoryByName(queryCategoryAdminConfig),
    placeholderData: keepPreviousData,
    enabled: isSearch
  });
  const totalPagesCategoryListSearch = Number(categoryListSearchData?.data.data.meta.totalPages);
  ...
  const handleClickButtonEdit = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>, categoryId: string) => {
  ...
  return (
    <div className='my-[54px]'>
      <div className='text-[18px] leading-7 font-medium text-[#18191c]'>Categories</div>
      ...
      <div className='mt-2 h-[530px]'>
        {categoryListLoading ? (
          ...
          categoryListSearchLoading ? (
            <div className='h-[530px] flex items-center justify-center  '>
              <Loading />
            </div>
          ) : categoryListSearchData?.data.data.data && categoryListSearchData?.data.data.data.length > 0 ? (
            categoryListSearchData?.data.data.data.map((category, index) => (
              <div
                key={category.id}
        ...
        hover:cursor-pointer hover:outline outline-[#0b65cc] rounded-[8px]'
              >
                ...
                <div className='col-span-5 text-[16px] text-[#535860]'>{category.id}</div>
                <div className='col-span-3 text-[16px] text-[#535860]'>{category.name}</div>
        ...
        ) : categoryListData?.data.data.data && categoryListData?.data.data.data.length > 0 ? (
          categoryListData?.data.data.data.map((category, index) => (
            <div
              key={category.id}
              className='grid grid-cols-12 px-[20px] py-[10px] items-center border-b
        hover:cursor-pointer hover:outline outline-[#0b65cc] rounded-[8px]'
      ...
      {categoryListLoading || categoryListSearchLoading ? (
        ...
        <div className='mt-[30px] flex items-center justify-end gap-x-[20px]'>
          {isSearch ? (
            <>
              ...
              {pageNo === totalPagesCategoryListSearch ? (
                <span className=''>Next</span>
              ) : (
                <Link
                  className='text-[#0b65cc]'
              ...
              <div className='w-[100px]'>
                Page {pageNo} / {totalPagesCategoryListSearch}
              </div>
            </>
          ) : (
            <>
              ...
              <div className='w-[100px]'>
                Page {pageNo} / {totalPagesCategoryList}
              </div>
            </>
          )}
        </div>
      )}
      {isOpenModalCreateCategory && <ModalCreateCategory closeModal={() => setOpenModalCreateCategory(false)} />}
      {isOpenModalUpdateCategory && (
        <ModalUpdateCategory categoryId={categoryId} closeModal={() => setOpenModalUpdateCategory(false)} />
      )}
    </div>
  );
}
